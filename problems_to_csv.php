<?php
  ...
  mysqli_close($conn);*/
  $file = 'id, unit, topic, difficulty, question, answer, random
';
  for($n=0;$n<10;$n++){
    $file = $file.$id[$n].','.$unit[$n].','.$topic[$n].','.$difficulty[$n].','.str_replace('
','',str_replace(',','&#44;',$problems[$n]))
      .','.str_replace(',','&#44;',$answers[$n]).','.$random[$n].'
';
  }
  file_put_contents('problem_data.csv', $file)
?>